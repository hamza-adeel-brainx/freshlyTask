...
//array maintainig the products in cart//
  ...
  let cartArray=[];     let totalPrice=0;
 ...
 function addToCart(e){
    ...
    if (cartArray.length > 0) {
        ...
        if (!itemExists) {
            let cartItemObj = { title: productTitle.textContent, Image: imgSrc, Price: price, Quantity: 1, special: false };
            if(primium)
            {
...
function calculatePriceUpdatePlan(type,price){
    ...
    totalPrice=totalPrice+Number(price);
        ...
        totalPrice=totalPrice-Number(price); const template = document.querySelector('.cart template');
